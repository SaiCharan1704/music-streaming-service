...
const playButtons = document.querySelectorAll(".cover-play-btn");
...
let homeSong = new Audio("Mawaa Enthaina.mp3");
...
function homeSongPlay() {
    ...
    if (homeSong.paused) {
        ...
        masterSongTitle.textContent = "Mawaa Enthaina";
        masterSongInfo.textContent = "Mahesh Babu,Sreeleela";
        masterCover.src = "Mawaa Enthaina pic.jpeg";
...
playBtnHome.addEventListener("click", () => {
  ...
  songCovers.forEach(card => {
    card.addEventListener("click", function () {
      const fileToOpen = this.getAttribute("data-file");
      window.location.href=fileToOpen; // Open in new tab
    });
  });
...
// follow button
followBtn.onclick = () => {
    window.open("https://www.youtube.com/watch?v=5yPO5lFeAIE");
}
window.addEventListener('DOMContentLoaded', () => {
    const params = new URLSearchParams(window.location.search);
    const songTitle = params.get('song');
    if (songTitle) {
      const songIndex = songList.findIndex(s => s.title === songTitle);
      if (songIndex !== -1) {
        loadSong(songIndex);
        playSong();
      }
  ...
  // ✅ Auto-play song if passed via URL
window.addEventListener("DOMContentLoaded", () => {
    const params = new URLSearchParams(window.location.search);
    const songTitle = params.get('song');
  
    if (songTitle) {
      const songIndex = songList.findIndex(song => song.title === songTitle);
      if (songIndex !== -1) {
  ...
  document.querySelectorAll(".cover-play-btn").forEach(btn => {
    btn.addEventListener("click", function (e) {
      ...
      fetch(file)
        ...
        .then(html => {
          ...
          // Optional: Extract song list and start playing a song
        });
    });
  });
